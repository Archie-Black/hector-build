...
4. **Reuse a base for consistency.** Default off in this repo. OS V01D / Hector: unique generations only (`.grok/references/unique-generation.md`). Do **not** reuse a prior `file_path` to save time. Fresh `imagine_text_to_image` unless the current user message grants permission to edit that one picture. Recurring characters still need unique instances in a scene (no cloned faces, horses, vehicles, flags, weapons).
