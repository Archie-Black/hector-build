...
function hxRules(lessons: string[]): string[] {
  ...
  return [
    ...
    "Bots may share a backend terminal (share_term) or SSH/PuTTY (share_link, share_ssh). Off-LAN bot SSH uses onion. Credentials live in the backend and are reused. Only allowlisted commands over the wire.",
